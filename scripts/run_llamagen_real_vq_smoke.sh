...
bash scripts/setup_llamagen_tokenizer.sh "$PROJECT_ROOT" "$REPO_DIR" "$CKPT_PATH"
...
for idx, color in enumerate([(240, 120, 40), (60, 120, 240)]):
    ...
    draw.ellipse((128, 128, 384, 384), outline=(255, 255, 255), width=10)
...
python -m qwen35_nano.precompute_tokens \
  ...
  --image-size 256 \
  --batch-size 2 \
  --tokenizer-backend llamagen_vq16_t2i \
  --llamagen-repo "$REPO_DIR" \
  --llamagen-ckpt "$CKPT_PATH" \
